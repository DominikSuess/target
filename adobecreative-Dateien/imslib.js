...
if(this._isAccessTokenInfoUsable(accessTokenInfo)){this._accessTokensInfos[key]=accessTokenInfo;}else{debugAlert("unusable access token info: "+_storage.getItem(key));_storage.removeItem(key);}}catch(e){_storage.removeItem(key);}}
...
var accessTokenKeys=Object_keys(this._accessTokensInfos);if(accessTokenKeys.length){var lastAccessTokenInfo=this._accessTokensInfos[accessTokenKeys.pop()];var countOfTokensToInvalidate=accessTokenKeys.length;if(countOfTokensToInvalidate===0){invalidateAccessTokenWithRedirect(lastAccessTokenInfo);}else{each(accessTokenKeys,function(index,accessTokenKey){self.makeApiCall(self.config.api.logout_token,{'access_token':self._accessTokensInfos[accessTokenKey].access_token},function(){--countOfTokensToInvalidate;if(countOfTokensToInvalidate===0){invalidateAccessTokenWithRedirect(lastAccessTokenInfo);}});});}}else{self._clearCredentials({"updateUI":true});}},"switchToRedirectMode":function(){this.adobeid.uses_modal_mode=false;this.adobeid.uses_redirect_mode=true;},"switchToModalMode":function(){this.adobeid.uses_modal_mode=true;this.adobeid.uses_redirect_mode=false;},"_clearCredentials":function(params){if(params&&params.access_token){var key=this.getKeyForAccessToken(params.access_token);_storage.removeItem(key);removeKeyFromObject(this._accessTokensInfos,key);}else{for(var key in this._accessTokensInfos)
    ...
    window.adobeIMS.config = {
        ...
        api: {
            ...
            "logout_token": {
                "name": "logout_token",
                "endpoint": {
                     "adobedotcom2": "https://ims-na1.adobelogin.com/ims/logout/v1/token"
